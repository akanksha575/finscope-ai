...
export default function ReportViewer({ report }: ReportViewerProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Report Header */}
      <div className="bg-fs-card rounded-2xl border border-fs-border shadow-card p-8">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-fs-highlight mb-4">
        ...
        <div className="border-t border-fs-border pt-6">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-fs-highlight mb-4">Executive Summary</h2>
            ...
            {report.report.key_findings && report.report.key_findings.length > 0 && (
              <div className="mt-6">
                <h3 className="text-lg font-semibold text-fs-highlight mb-3">Key Takeaways:</h3>
      ...
      {report.report.key_findings && report.report.key_findings.length > 4 && (
        <div className="bg-fs-card rounded-2xl border border-fs-border shadow-card p-8">
          <h2 className="text-2xl font-bold text-fs-highlight mb-4">Key Findings</h2>
          <ul className="space-y-3">
            {report.report.key_findings.slice(4).map((finding, index) => (
              <li key={index} className="flex items-start gap-3">
                <span className="text-zinc-400 mt-1 font-bold">•</span>
      ...
      {report.report.recommendations && report.report.recommendations.length > 0 && (
        <div className="bg-fs-card rounded-2xl border border-fs-border shadow-card p-8">
          <h2 className="text-2xl font-bold text-fs-highlight mb-4">Recommendations</h2>
          <ul className="space-y-3">
      ...
      {report.report.financial_highlights && report.report.financial_highlights.length > 0 && (
        <div className="bg-fs-card rounded-2xl border border-fs-border shadow-card p-8">
          <h2 className="text-2xl font-bold text-fs-highlight mb-4">Financial Highlights</h2>
          <ul className="space-y-3">
            {report.report.financial_highlights.map((highlight, index) => (
              <li key={index} className="text-zinc-300">{highlight}</li>
            ))}
          </ul>
        </div>
      )}
